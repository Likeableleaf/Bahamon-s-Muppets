...
if instance_nearest(x,y,obj_studentInDesk).inDesk == true {
	if (buffer_time == 0) {
		...
		if (instance_nearest(x,y,obj_studentInDesk).id != owner && owner != noone) {
			...
			// boot player
			instance_nearest(x,y,obj_studentInDesk).direction = point_direction(x, y, instance_nearest(x,y,obj_simpleStudent).x, instance_nearest(x,y,obj_simpleStudent).y)
